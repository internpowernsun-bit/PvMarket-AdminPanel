...
@section('title', $mode === 'create' ? 'Create Price Promotion' : 'Edit Price Promotion')
...
<style>
    ...
    .form-sub-grid  { display:grid; grid-template-columns:1fr 1fr; gap:20px; margin-bottom:20px; }
    ...
    .img-preview { height:50px; border-radius:6px; border:1px solid var(--border); object-fit:cover; display:block; margin-bottom:8px; }

    .section-divider { border:none; border-top:1px solid var(--border); margin:20px 0; }

    /* Quill wrapper — same as blogs page */
...
{{-- Quill CSS loaded inside content — same pattern as blogs.blade.php --}}
...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:20px;">
    <h1 style="font-size:22px; font-weight:800; color:var(--text);">{{ $mode === 'create' ? 'Create New Price Promotions Details' : 'Edit Price Promotion' }}</h1>
    <a href="{{ route('admin.knowledge-hub.price-promotions.index') }}" class="btn-back">← Back</a>
</div>
...
<div class="content-panel">
    ...
    <form
        method="POST"
        action="{{ $mode === 'create'
            ? route('admin.knowledge-hub.price-promotions.store')
            : route('admin.knowledge-hub.price-promotions.update', $record->id) }}"
    ...
    >
        ...
        <div class="form-main-grid">
            ...
            <div class="form-group">
                ...
                <input
                    ...
                    name="heading"
                    class="form-input"
                    placeholder="e.g. Weekly pv.market Stock Highlights"
        ...
        <div class="form-sub-grid">
            ...
            <div class="form-group">
                ...
                <input
                    ...
                    name="event_place"
                    class="form-input"
                    placeholder="e.g. Stock update"
                    value="{{ old('event_place', $record->event_place ?? '') }}"
                />
            </div>
        ...
        {{-- Description — Quill editor (same pattern as blogs page) --}}
...
{{-- Quill JS + init — same pattern as blogs page --}}
...
@section('title', 'Price Promotions')
...
<style>
    ...
    .promotion-thumb { width:90px; height:56px; border-radius:6px; object-fit:cover; border:1px solid var(--border); display:block; margin:0 auto; }
...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:20px;">
    <h1 style="font-size:22px; font-weight:800; color:var(--text);">Price Promotions</h1>
    <a href="{{ route('admin.knowledge-hub.price-promotions.create') }}"
...
<div class="content-panel">
    <div class="table-toolbar">
        <form method="GET" action="{{ route('admin.knowledge-hub.price-promotions.index') }}" class="search-group">
            <label>Search:</label>
        ...
        <form method="GET" action="{{ route('admin.knowledge-hub.price-promotions.index') }}" class="entries-group">
            Show
    ...
    {{ Str::limit(strip_tags(lang($promo, 'description')), 60) ?: '—' }}
                ...
                <td class="center">
                    @if($promo->image)
                        <img src="{{ asset('storage/' . $promo->image) }}"
                             class="promotion-thumb" alt="{{ $promo->heading }}"/>
                    @else
                ...
                <td>
                    <div class="action-btns">
                        <a href="{{ route('admin.knowledge-hub.price-promotions.edit', $promo->id) }}"
                        ...
                        <button class="action-icon delete" title="Delete"
                            onclick="if(confirm('Delete this promotion?')) document.getElementById('del-{{ $promo->id }}').submit();">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        ...
                        <form id="del-{{ $promo->id }}" method="POST"
                              action="{{ route('admin.knowledge-hub.price-promotions.destroy', $promo->id) }}"
                              style="display:none;">
                            @csrf @method('DELETE')
                        </form>
                    </div>
            ...
            <tr>
                <td colspan="7">
                    <div class="empty-state">
                        ...
                        <p>No price promotions yet. Click <strong>Add +</strong> to create one.</p>
                    </div>
                </td>
            </tr>
            @endforelse
    ...
    <span>{{ $promotions->firstItem() ?? 0 }}–{{ $promotions->lastItem() ?? 0 }} of {{ $promotions->total() }} entries</span>
    ...
    <nav style="display:flex; align-items:center; gap:4px;">
        ...
        @else
            <a href="{{ $promotions->previousPageUrl() }}" style="display:flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:6px;border:1.5px solid var(--border);background:white;color:var(--text);text-decoration:none;font-size:16px;font-weight:600;transition:all .15s;" onmouseover="this.style.borderColor='var(--primary)';this.style.color='var(--primary)';this.style.background='var(--primary-l)';" onmouseout="this.style.borderColor='var(--border)';this.style.color='var(--text)';this.style.background='white';">‹</a>
        @endif
        ...
        @foreach ($promotions->getUrlRange(1, $promotions->lastPage()) as $page => $url)
            ...
            @elseif ($page == 1 || $page == $promotions->lastPage() || abs($page - $promotions->currentPage()) <= 2)
            ...
            @elseif ($page == $promotions->currentPage() - 3 || $page == $promotions->currentPage() + 3)
        ...
        @if ($promotions->hasMorePages())
            <a href="{{ $promotions->nextPageUrl() }}" style="display:flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:6px;border:1.5px solid var(--border);background:white;color:var(--text);text-decoration:none;font-size:16px;font-weight:600;transition:all .15s;" onmouseover="this.style.borderColor='var(--primary)';this.style.color='var(--primary)';this.style.background='var(--primary-l)';" onmouseout="this.style.borderColor='var(--border)';this.style.color='var(--text)';this.style.background='white';">›</a>
        @else
            <span style="display:flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:6px;border:1.5px solid var(--border);background:white;color:#CBD5E1;cursor:not-allowed;font-size:16px;">›</span>
        @endif
    </nav>
